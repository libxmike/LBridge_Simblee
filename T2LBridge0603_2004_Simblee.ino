...
void loop()
{
  ...
  Serial.print(" ==="); Serial.printf(" RSSI: %f", SimbleeData.rssi);
  Serial.print(" =================================================");
...
void setupBluetoothConnection()
{
  ...
  SimbleeBLE.advertisementData = "rfduino";
...
void forLimiTTer()
{
  ...
  TxBuffer1 += " ";
  TxBuffer1 += String(SimbleeData.voltage);
  TxBuffer1 += " ";
  TxBuffer1 += String(SimbleeData.voltagePercent);
  TxBuffer1 += " ";
...
void forTransmiter1()
{
  ...
  TxBuffer1 += String(SimbleeData.voltagePercent);
  TxBuffer1 += " ";
  TxBuffer1 += String((int)(SimbleeData.temperatureC * 10));
  int  LL = TxBuffer1.length();
#ifdef DEBUG
...
void forTransmiter2()
{
  ...
  // SimbleeData.voltage, SimbleeData.voltagePercent, SimbleeData.temperatureC,
  //  RSSI
  ...
  allHardware +=  String(SimbleeData.voltage) + " ";
  allHardware +=  String(SimbleeData.voltagePercent) + " ";
  allHardware +=  String((int)(SimbleeData.temperatureC * 10)) + " ";
  allHardware +=  String((int)(SimbleeData.rssi)) + " ";
...
void forLibreCGM()
{
  ...
  batteryData.voltage = (float) SimbleeData.voltage;
...
void Sim_bleeData()
{
  print_state("SimbleeData(): ");

  analogReference(VBG);
  analogSelection(VDD_1_3_PS);
  int sensorValue = analogRead(1);
  ///SimbleeData.voltage = sensorValue * (360 / 1023.0) * 10;
   SimbleeData.voltage = sensorValue * 20.45; // * (3.6 / 1023.0);
  SimbleeData.voltagePercent = map(SimbleeData.voltage, MIN_VOLTAGE, MAX_VOLTAGE, 0, 100);
  SimbleeData.temperatureC = Simblee_temperature(CELSIUS);
  SimbleeData.temperatureF = Simblee_temperature(FAHRENHEIT);
  if (SimbleeData.voltage < 2400) BatteryOK = false;
  else BatteryOK = true;

#ifdef DEBUGM
  Serial.print(" - U[mv]: ");
  Serial.print(SimbleeData.voltage);
  Serial.print(" - U[%]: ");
  Serial.print(SimbleeData.voltagePercent);
  Serial.print(" - Temp[C]: ");
  Serial.print(SimbleeData.temperatureC);
  Serial.print(" - Temp[F]: ");
  Serial.print(SimbleeData.temperatureF);
  Serial.print(" - RSSI: ");
  Serial.print(SimbleeData.rssi);
#endif
}
...
//  print_state("print_packet()");
  ...
  msg.my_battery = SimbleeData.voltagePercent;
